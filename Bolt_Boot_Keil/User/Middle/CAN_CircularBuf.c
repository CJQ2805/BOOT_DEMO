...
int can_rx_data(uint8_t u8can_channel ,void * pvcan_rx_header, u8* pu8data)
{
	...
	tpacket.u_u32id_field.u32id = FDCAN_RxHeader->Identifier;
...
void can_circulbuf_init(void)
{
	...
	gt_STDID_can1_circul_buf.ptcircul_packet = malloc(sizeof(t_can_packet)*CAN1_CIRCUL_BUF_LEN);//(pt_can_packet)au8data1;//
